...
async function run() {
  try {
    ...
    if (!tableInfo.vacancies) {
      console.log('Column "vacancies" is missing. Altering table...');
      await sequelize.query('ALTER TABLE `careers` ADD COLUMN `vacancies` VARCHAR(255) NULL AFTER `post`;');
    ...
    if (!tableInfo.experience) {
      console.log('Column "experience" is missing. Altering table...');
      await sequelize.query('ALTER TABLE `careers` ADD COLUMN `experience` VARCHAR(255) NULL AFTER `vacancies`;');
