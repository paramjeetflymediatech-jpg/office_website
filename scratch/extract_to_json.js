...
for (const [pid, data] of Object.entries(posts)) {
    ...
    const slug = String(data._wp_old_slug || '').toLowerCase();
    
    const hasKeyword = citiesAndCountries.some(keyword => title.includes(keyword) || desc.includes(keyword) || slug.includes(keyword));
    ...
    if (hasKeyword && hasContent) {
        filteredList.push({
            ...
            oldSlug: data._wp_old_slug || null
        });
    }
}

// Write full list to scratch JSON
fs.writeFileSync(path.join(__dirname, 'canada_seo_candidates.json'), JSON.stringify(filteredList, null, 2));
console.log(`Saved ${filteredList.length} candidate entries to scratch/canada_seo_candidates.json.`);

// Print simplified list
console.log('--- Simplified Candidate List ---');
filteredList.forEach(item => {
    console.log(`ID: ${item.postId} | Title: "${item.title}" | OldSlug: "${item.oldSlug}"`);
});
